...
def wsop():
    ...
    cursor.execute("SELECT cast(po_header.order_date as date), po_detail.order_quantity, po_detail.unit_cost, po_header.trade_currency from po_header inner join po_detail on po_header.po = po_detail.po where po_header.status not like 'unissued' and po_detail.po not like '19981' and po_detail.po not like '20200' and po_header.order_date > Dateadd(year, -1, getdate()) order by po_header.order_date desc")
...
def production_review(name=None):
    ...
    graph_data = [{'categorie': 'LASER', 'values': [{'value': 0, 'rate': 'Job Count'}, {'value': 0, 'rate': 'Hour Count'}]},{'categorie': 'TOYOKOKI', 'values': [{'value': 0, 'rate': 'Job Count'}, {'value': 0, 'rate': 'Hour Count'}]}, {'categorie': 'WELDING', 'values': [{'value': 0, 'rate': 'Job Count'}, {'value': 0, 'rate': 'Hour Count'}]}, {'categorie': 'SHOP', 'values': [{'value': 0, 'rate': 'Job Count'}, {'value': 0, 'rate': 'Hour Count'}]}]

    for each in change_data:
        my_item = next((item for item in graph_data if item['categorie'] == each[1]), None)
        job_c = next((item for item in my_item['values'] if  item['rate'] == 'Job Count'), None)
        job_c['value'] += 1
        hour_c = next((item for item in my_item['values'] if  item['rate'] == 'Hour Count'), None)
        hour_c['value'] += each[2]
...
def wso():
    ...
    week_order = list(table1)[:-5]

    values_order1 = table1.values.tolist()
    values_order1 = [float(x) for x in values_order1[0]][:-5]
    i = len(values_order1)
    d1 = pd.DataFrame({
    'week' : [float(x) for x in range(1,i+1)],
    'value' : values_order1
