...
def ato():
    ...
    cursor.execute("select job.job, job.total_price, cast(job.order_date as date) from job where Job.Customer Not Like '%GARAGESCAP%' And Job.Customer Not Like '%I-H%' AND Job.Job Not Like '%-%' and job.order_date > DATEADD(year, DATEDIFF(year, 0, getDate() - 3), 0)")
    query = [list(x) for x in cursor.fetchall()]
    weekly_hours_data = []
    for job in query:
