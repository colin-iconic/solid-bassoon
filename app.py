...
def search(name=None):
	...
	#get data from quote
	cursor.execute("select rfq.comments, rfq.contact, rfq.customer, rfq.note_text, rfq.reference, quote.comment, quote.description, quote.ext_description, quote.line, quote.note_text, quote.part_number, quote.reference, quote.rfq, quote.quoted_by from quote inner join rfq on quote.rfq = rfq.rfq where (rfq.comments like '%{0}%' or rfq.contact like '%{0}%' or rfq.customer like '%{0}%' or rfq.note_text like '%{0}%' or rfq.reference like '%{0}%' or quote.comment like '%{0}%' or quote.description like '%{0}%' or quote.ext_description like '%{0}%' or quote.line like '%{0}%' or quote.note_text like '%{0}%' or quote.part_number like '%{0}%' or quote.reference like '%{0}%' or quote.rfq like '%{0}%' or quote.quoted_by like '%{0}%') and quote.status like '%{1}%'".format(search, status))

	data['quote'] = [list(x) for x in cursor.fetchall()]
	data['quote_head'] = ['Comments', 'Contact', 'Customer', 'Note Text', 'Reference', 'Comment', 'Description', 'Ext. Description', 'Line', 'Note Text', 'Part Number', 'Reference', 'RFQ', 'Quoted By']
